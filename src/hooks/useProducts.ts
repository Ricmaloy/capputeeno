...
export async function getProducts(
  ...
  const formatedFilter =
    sortField === 'all' ? {} : { category: `${sortField}` };
  const formatedOrder =
    sortOrder === 'none' ? '' : [...formatOrderList[`${sortOrder}`]];
...
export function useProducts(
  ...
  return useQuery(
    ['@capputeeno:products', sortField, sortOrder, page],
    () => getProducts(sortField, sortOrder, page),
    {
