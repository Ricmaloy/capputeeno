...
export default function Cart() {
  ...
  const { cart, cartSize, cartTotal, handleBuyProducts, isFreeFreight } =
    useCart();
  ...
  function handleBuyCart() {
    ...
    toast.success(
      `${successIcons[firstIcon]} Compra realizada ! ${successIcons[secondIcon]}`,
      toastOptions,
    );
        ...
        <CheckoutContainer>
          ...
          <CheckoutSection>
            ...
            <PriceContainer isFreefreight={isFreeFreight}>
              <p>R$ 40,00</p>
              {isFreeFreight && (
                <span aria-label="Cart free freight">gratuito</span>
              )}
            </PriceContainer>
          </CheckoutSection>
          ...
          <CheckoutSection>
            <h3>Total</h3>
            {isFreeFreight ? (
              <h3 aria-label="Cart total price with free freight">
                {formatPrice(cartTotal)}
              </h3>
            ) : (
              <h3 aria-label="Cart total price without free freight">
                {formatPrice(cartTotal + 4000)}
              </h3>
            )}
          </CheckoutSection>
          ...
          {cart.length === 0 ? (
            ...
            <CheckoutButton onClick={handleBuyCart}>
              Finalizar a compra
            </CheckoutButton>
          )}
