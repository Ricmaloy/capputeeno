...
export default function Products() {
  ...
  const { data, isLoading, error } = useSearch(0, `${slug}`);
  ...
  return (
    <>
      ...
      <Container>
        <Title>
          Você pesquisou por: <span>{slug}</span>
        </Title>
        {isLoading ? (
          <ProductsCardDisplayShimmer />
        ) : error ? (
          <Subtitle>
            Ops ! Algo deu errado, <span>tente novamente.</span>
          </Subtitle>
        ) : data.products.length > 0 ? (
          <>
            <Subtitle>
              Encontramos <span>{data.count}</span> produtos para você !
            </Subtitle>
          ...
          <Subtitle>
            Não encontramos nenhum que condiz com sua busca,{' '}
            <span>tente algo diferente.</span>
          </Subtitle>
        )}
      </Container>
    </>
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  await queryClient.prefetchQuery('@capputeeno:searches', () =>
    searchProducts(0, `${slug}`),
  );
